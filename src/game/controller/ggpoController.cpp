...
GGPOController::GGPOController(gvl::shared_ptr<Common> common, gvl::shared_ptr<Settings> settings)
...
GGPOController::~GGPOController()
{
	endRecord();
}

void GGPOController::onKey(int key, bool keyState)
{
...
void GGPOController::draw(Renderer& renderer, bool useSpectatorViewports)
{
	if(state == StateWeaponSelection)
	{
...
void GGPOController::changeState(GameState newState)
{
	if(state == newState)
		return;
...
void GGPOController::swapLevel(Level& newLevel)
{
	currentLevel()->swap(newLevel);
}

Level* GGPOController::currentLevel()
{
	return &game.level;
}

Game* GGPOController::currentGame()
{
	return &game;
}
	
bool GGPOController::running()
{
